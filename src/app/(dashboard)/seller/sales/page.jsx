...
export default function SellerSales() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "hold":
        return (
          <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-xs font-black uppercase flex items-center gap-1">
            <Clock size={12} /> In Escrow
          </span>
        );
      case "released":
        return (
          <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-black uppercase flex items-center gap-1">
            <CheckCircle2 size={12} /> Paid
          </span>
        );
      case "disputed":
        return (
          <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-black uppercase flex items-center gap-1">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          <h1 className="text-4xl font-black text-slate-900">
            Recent <span className="text-amber-500">Sales</span>
          </h1>
          <p className="text-slate-500 mt-1 font-medium">
            Monitor your orders and escrow status here.
          </p>
        </div>
      ...
      <div className="grid gap-6">
        {sales.length === 0 ? (
          ...
          sales.map((sale, index) => (
            <div
              key={sale._id || sale.tran_id || index}
              className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm hover:shadow-md transition-all group flex flex-col md:flex-row justify-between items-center gap-6"
            >
              <div className="flex items-center gap-5 w-full md:w-auto">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-900">
                    {sale.product?.name || "Premium Product"}
                  </h2>
              ...
              <div className="flex items-center gap-8 w-full md:w-auto justify-between md:justify-end">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {sale.escrowStatus === "hold" && (
                    <p className="text-[9px] text-amber-600 font-bold animate-pulse italic">
